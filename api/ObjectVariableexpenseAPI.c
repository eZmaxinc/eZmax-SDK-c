...
ObjectVariableexpenseAPI_variableexpenseDeleteObjectV1(apiClient_t *apiClient, int pkiVariableexpenseID )
{
    ...
    //nonprimitive not container
    cJSON *ObjectVariableexpenseAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    variableexpense_delete_object_v1_response_t *elementToReturn = variableexpense_delete_object_v1_response_parseFromJSON(ObjectVariableexpenseAPIlocalVarJSON);
    cJSON_Delete(ObjectVariableexpenseAPIlocalVarJSON);
    if(elementToReturn == NULL) {
        // return 0;
    }
