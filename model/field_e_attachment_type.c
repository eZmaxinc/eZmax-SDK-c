...
ezmax_api_definition__full_field_e_attachment_type__e field_e_attachment_type_parseFromJSON(cJSON *field_e_attachment_typeJSON) {
    if(!cJSON_IsString(field_e_attachment_typeJSON) || (field_e_attachment_typeJSON->valuestring == NULL)) {
        return 0;
    }
    return field_e_attachment_type_field_e_attachment_type_FromString(field_e_attachment_typeJSON->valuestring);
}
