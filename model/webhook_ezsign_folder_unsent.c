...
#include <string.h>
#include <stdio.h>
#include "webhook_ezsign_folder_unsent.h"



static webhook_ezsign_folder_unsent_t *webhook_ezsign_folder_unsent_create_internal(
    custom_webhook_response_t *obj_webhook,
    list_t *a_obj_attempt,
    ezsignfolder_response_t *obj_ezsignfolder
    ) {
    webhook_ezsign_folder_unsent_t *webhook_ezsign_folder_unsent_local_var = malloc(sizeof(webhook_ezsign_folder_unsent_t));
...
__attribute__((deprecated)) webhook_ezsign_folder_unsent_t *webhook_ezsign_folder_unsent_create(
    custom_webhook_response_t *obj_webhook,
    list_t *a_obj_attempt,
    ezsignfolder_response_t *obj_ezsignfolder
    ) {
...
void webhook_ezsign_folder_unsent_free(webhook_ezsign_folder_unsent_t *webhook_ezsign_folder_unsent) {
    if(NULL == webhook_ezsign_folder_unsent){
        return ;
    }
    if(webhook_ezsign_folder_unsent->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "webhook_ezsign_folder_unsent_free");
        return ;
    }
    listEntry_t *listEntry;
    if (webhook_ezsign_folder_unsent->obj_webhook) {
        custom_webhook_response_free(webhook_ezsign_folder_unsent->obj_webhook);
        webhook_ezsign_folder_unsent->obj_webhook = NULL;
    }
    if (webhook_ezsign_folder_unsent->a_obj_attempt) {
        list_ForEach(listEntry, webhook_ezsign_folder_unsent->a_obj_attempt) {
            attempt_response_compound_free(listEntry->data);
        }
        list_freeList(webhook_ezsign_folder_unsent->a_obj_attempt);
        webhook_ezsign_folder_unsent->a_obj_attempt = NULL;
    }
    if (webhook_ezsign_folder_unsent->obj_ezsignfolder) {
        ezsignfolder_response_free(webhook_ezsign_folder_unsent->obj_ezsignfolder);
        webhook_ezsign_folder_unsent->obj_ezsignfolder = NULL;
    }
    free(webhook_ezsign_folder_unsent);
}

cJSON *webhook_ezsign_folder_unsent_convertToJSON(webhook_ezsign_folder_unsent_t *webhook_ezsign_folder_unsent) {
    cJSON *item = cJSON_CreateObject();

    // webhook_ezsign_folder_unsent->obj_webhook
    if (!webhook_ezsign_folder_unsent->obj_webhook) {
        goto fail;
    }
    cJSON *obj_webhook_local_JSON = custom_webhook_response_convertToJSON(webhook_ezsign_folder_unsent->obj_webhook);
    ...
    // webhook_ezsign_folder_unsent->a_obj_attempt
    if (!webhook_ezsign_folder_unsent->a_obj_attempt) {
        goto fail;
    }
    ...
    if (webhook_ezsign_folder_unsent->a_obj_attempt) {
    list_ForEach(a_obj_attemptListEntry, webhook_ezsign_folder_unsent->a_obj_attempt) {
    ...
    // webhook_ezsign_folder_unsent->obj_ezsignfolder
    if (!webhook_ezsign_folder_unsent->obj_ezsignfolder) {
        goto fail;
    }
    cJSON *obj_ezsignfolder_local_JSON = ezsignfolder_response_convertToJSON(webhook_ezsign_folder_unsent->obj_ezsignfolder);
...
webhook_ezsign_folder_unsent_t *webhook_ezsign_folder_unsent_parseFromJSON(cJSON *webhook_ezsign_folder_unsentJSON){
    ...
    webhook_ezsign_folder_unsent_t *webhook_ezsign_folder_unsent_local_var = NULL;

    // define the local variable for webhook_ezsign_folder_unsent->obj_webhook
    custom_webhook_response_t *obj_webhook_local_nonprim = NULL;

    // define the local list for webhook_ezsign_folder_unsent->a_obj_attempt
    list_t *a_obj_attemptList = NULL;

    // define the local variable for webhook_ezsign_folder_unsent->obj_ezsignfolder
    ezsignfolder_response_t *obj_ezsignfolder_local_nonprim = NULL;

    // webhook_ezsign_folder_unsent->obj_webhook
    cJSON *obj_webhook = cJSON_GetObjectItemCaseSensitive(webhook_ezsign_folder_unsentJSON, "objWebhook");
    ...
    // webhook_ezsign_folder_unsent->a_obj_attempt
    cJSON *a_obj_attempt = cJSON_GetObjectItemCaseSensitive(webhook_ezsign_folder_unsentJSON, "a_objAttempt");
    ...
    // webhook_ezsign_folder_unsent->obj_ezsignfolder
    cJSON *obj_ezsignfolder = cJSON_GetObjectItemCaseSensitive(webhook_ezsign_folder_unsentJSON, "objEzsignfolder");
