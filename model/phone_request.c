...
static phone_request_t *phone_request_create_internal(
    ...
    memset(phone_request_local_var, 0, sizeof(phone_request_t));
...
__attribute__((deprecated)) phone_request_t *phone_request_create(
    ...
    phone_request_t *result = phone_request_create_internal (
        pki_phone_id_copy,
...
void phone_request_free(phone_request_t *phone_request) {
    ...
    if (phone_request->pki_phone_id) {
        free(phone_request->pki_phone_id);
        phone_request->pki_phone_id = NULL;
    }
    if (phone_request->fki_phonetype_id) {
        free(phone_request->fki_phonetype_id);
        phone_request->fki_phonetype_id = NULL;
    }
...
phone_request_t *phone_request_parseFromJSON(cJSON *phone_requestJSON){
    ...
    // define the local variable for phone_request->pki_phone_id
    int *pki_phone_id_local_var = NULL;

    // define the local variable for phone_request->fki_phonetype_id
