...
void paymentterm_request_free(paymentterm_request_t *paymentterm_request) {
    ...
    if(paymentterm_request->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "paymentterm_request_free");
        return ;
    }
...
cJSON *paymentterm_request_convertToJSON(paymentterm_request_t *paymentterm_request) {
    ...
    if (ezmax_api_definition__full_field_e_paymentterm_type__NULL == paymentterm_request->e_paymentterm_type) {
        goto fail;
    }
