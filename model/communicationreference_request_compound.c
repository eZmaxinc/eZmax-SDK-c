...
static communicationreference_request_compound_t *communicationreference_request_compound_create_internal(
    ...
    memset(communicationreference_request_compound_local_var, 0, sizeof(communicationreference_request_compound_t));
    communicationreference_request_compound_local_var->_library_owned = 1;
...
__attribute__((deprecated)) communicationreference_request_compound_t *communicationreference_request_compound_create(
    ...
    communicationreference_request_compound_t *result = communicationreference_request_compound_create_internal (
...
void communicationreference_request_compound_free(communicationreference_request_compound_t *communicationreference_request_compound) {
    ...
    if(communicationreference_request_compound->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "communicationreference_request_compound_free");
        return ;
    }
    listEntry_t *listEntry;
    if (communicationreference_request_compound->pki_communicationreference_id) {
        free(communicationreference_request_compound->pki_communicationreference_id);
        communicationreference_request_compound->pki_communicationreference_id = NULL;
    }
    if (communicationreference_request_compound->fki_buyercontract_id) {
        free(communicationreference_request_compound->fki_buyercontract_id);
        communicationreference_request_compound->fki_buyercontract_id = NULL;
    }
    if (communicationreference_request_compound->fki_ezsignfolder_id) {
        free(communicationreference_request_compound->fki_ezsignfolder_id);
        communicationreference_request_compound->fki_ezsignfolder_id = NULL;
    }
    if (communicationreference_request_compound->fki_inscription_id) {
        free(communicationreference_request_compound->fki_inscription_id);
        communicationreference_request_compound->fki_inscription_id = NULL;
    }
    if (communicationreference_request_compound->fki_inscriptiontemp_id) {
        free(communicationreference_request_compound->fki_inscriptiontemp_id);
        communicationreference_request_compound->fki_inscriptiontemp_id = NULL;
    }
    if (communicationreference_request_compound->fki_invoice_id) {
        free(communicationreference_request_compound->fki_invoice_id);
        communicationreference_request_compound->fki_invoice_id = NULL;
    }
    if (communicationreference_request_compound->fki_otherincome_id) {
        free(communicationreference_request_compound->fki_otherincome_id);
        communicationreference_request_compound->fki_otherincome_id = NULL;
    }
    if (communicationreference_request_compound->fki_electronicfundstransfer_id) {
        free(communicationreference_request_compound->fki_electronicfundstransfer_id);
        communicationreference_request_compound->fki_electronicfundstransfer_id = NULL;
    }
    if (communicationreference_request_compound->fki_rejectedoffertopurchase_id) {
        free(communicationreference_request_compound->fki_rejectedoffertopurchase_id);
        communicationreference_request_compound->fki_rejectedoffertopurchase_id = NULL;
    }
...
communicationreference_request_compound_t *communicationreference_request_compound_parseFromJSON(cJSON *communicationreference_request_compoundJSON){
    ...
    // define the local variable for communicationreference_request_compound->pki_communicationreference_id
    int *pki_communicationreference_id_local_var = NULL;

    // define the local variable for communicationreference_request_compound->fki_buyercontract_id
    int *fki_buyercontract_id_local_var = NULL;

    // define the local variable for communicationreference_request_compound->fki_ezsignfolder_id
    int *fki_ezsignfolder_id_local_var = NULL;

    // define the local variable for communicationreference_request_compound->fki_inscription_id
    int *fki_inscription_id_local_var = NULL;

    // define the local variable for communicationreference_request_compound->fki_inscriptiontemp_id
    int *fki_inscriptiontemp_id_local_var = NULL;

    // define the local variable for communicationreference_request_compound->fki_invoice_id
    int *fki_invoice_id_local_var = NULL;

    // define the local variable for communicationreference_request_compound->fki_otherincome_id
    int *fki_otherincome_id_local_var = NULL;

    // define the local variable for communicationreference_request_compound->fki_electronicfundstransfer_id
    int *fki_electronicfundstransfer_id_local_var = NULL;

    // define the local variable for communicationreference_request_compound->fki_rejectedoffertopurchase_id
    ...
    *fki_rejectedoffertopurchase_id_local_var = fki_rejectedoffertopurchase_id->valuedouble;
    ...
    communicationreference_request_compound_local_var = communicationreference_request_compound_create_internal (
