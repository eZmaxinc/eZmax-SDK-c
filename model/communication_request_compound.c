...
void communication_request_compound_free(communication_request_compound_t *communication_request_compound) {
    ...
    if(communication_request_compound->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "communication_request_compound_free");
        return ;
    }
...
cJSON *communication_request_compound_convertToJSON(communication_request_compound_t *communication_request_compound) {
    ...
    if(communication_request_compound->e_communication_importance != ezmax_api_definition__full_field_e_communication_importance__NULL) {
    ...
    if (ezmax_api_definition__full_field_e_communication_type__NULL == communication_request_compound->e_communication_type) {
        goto fail;
    }
    ...
    if(cJSON_AddStringToObject(item, "eCommunicationAttachmenttype", communication_request_compound_e_communication_attachmenttype_ToString(communication_request_compound->e_communication_attachmenttype)) == NULL)
    {
    goto fail; //Enum
    }
    }
...
communication_request_compound_t *communication_request_compound_parseFromJSON(cJSON *communication_request_compoundJSON){
    ...
    if (cJSON_IsNull(a_obj_communicationattachment)) {
    ...
    if (cJSON_IsNull(a_obj_communicationrecipient)) {
    ...
    if (cJSON_IsNull(a_obj_communicationreference)) {
    ...
    if (cJSON_IsNull(a_obj_communicationexternalrecipient)) {
