...
static supply_response_t *supply_response_create_internal(
    ...
    memset(supply_response_local_var, 0, sizeof(supply_response_t));
...
__attribute__((deprecated)) supply_response_t *supply_response_create(
    ...
    supply_response_t *result = supply_response_create_internal (
        pki_supply_id_copy,
...
void supply_response_free(supply_response_t *supply_response) {
    ...
    if (supply_response->pki_supply_id) {
        free(supply_response->pki_supply_id);
        supply_response->pki_supply_id = NULL;
    }
    if (supply_response->fki_glaccount_id) {
        free(supply_response->fki_glaccount_id);
        supply_response->fki_glaccount_id = NULL;
    }
    if (supply_response->fki_glaccountcontainer_id) {
        free(supply_response->fki_glaccountcontainer_id);
        supply_response->fki_glaccountcontainer_id = NULL;
    }
    if (supply_response->fki_variableexpense_id) {
        free(supply_response->fki_variableexpense_id);
        supply_response->fki_variableexpense_id = NULL;
    }
    ...
    if (supply_response->b_supply_isactive) {
        free(supply_response->b_supply_isactive);
        supply_response->b_supply_isactive = NULL;
    }
    if (supply_response->b_supply_variableprice) {
        free(supply_response->b_supply_variableprice);
        supply_response->b_supply_variableprice = NULL;
    }
...
supply_response_t *supply_response_parseFromJSON(cJSON *supply_responseJSON){
    ...
    // define the local variable for supply_response->pki_supply_id
    int *pki_supply_id_local_var = NULL;

    // define the local variable for supply_response->fki_glaccount_id
    int *fki_glaccount_id_local_var = NULL;

    // define the local variable for supply_response->fki_glaccountcontainer_id
    int *fki_glaccountcontainer_id_local_var = NULL;

    // define the local variable for supply_response->fki_variableexpense_id
    ...
    // define the local variable for supply_response->b_supply_isactive
    int *b_supply_isactive_local_var = NULL;

    // define the local variable for supply_response->b_supply_variableprice
