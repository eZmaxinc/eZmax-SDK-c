...
static signature_response_t *signature_response_create_internal(
    ...
    memset(signature_response_local_var, 0, sizeof(signature_response_t));
...
__attribute__((deprecated)) signature_response_t *signature_response_create(
    ...
    signature_response_t *result = signature_response_create_internal (
...
void signature_response_free(signature_response_t *signature_response) {
    ...
    if (signature_response->pki_signature_id) {
        free(signature_response->pki_signature_id);
        signature_response->pki_signature_id = NULL;
    }
    if (signature_response->fki_font_id) {
        free(signature_response->fki_font_id);
        signature_response->fki_font_id = NULL;
    }
...
signature_response_t *signature_response_parseFromJSON(cJSON *signature_responseJSON){
    ...
    // define the local variable for signature_response->pki_signature_id
    int *pki_signature_id_local_var = NULL;

    // define the local variable for signature_response->fki_font_id
