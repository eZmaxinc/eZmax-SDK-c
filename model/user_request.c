...
static user_request_t *user_request_create_internal(
    ...
    memset(user_request_local_var, 0, sizeof(user_request_t));
    ...
    user_request_local_var->b_user_ezsigntemplaterolegrouping = b_user_ezsigntemplaterolegrouping;
...
__attribute__((deprecated)) user_request_t *user_request_create(
    ...
    user_request_t *result = user_request_create_internal (
        pki_user_id_copy,
        fki_agent_id_copy,
        fki_broker_id_copy,
...
void user_request_free(user_request_t *user_request) {
    ...
    if (user_request->pki_user_id) {
        free(user_request->pki_user_id);
        user_request->pki_user_id = NULL;
    }
    if (user_request->fki_agent_id) {
        free(user_request->fki_agent_id);
        user_request->fki_agent_id = NULL;
    }
    if (user_request->fki_broker_id) {
        free(user_request->fki_broker_id);
        user_request->fki_broker_id = NULL;
    }
    if (user_request->fki_assistant_id) {
        free(user_request->fki_assistant_id);
        user_request->fki_assistant_id = NULL;
    }
    if (user_request->fki_employee_id) {
        free(user_request->fki_employee_id);
        user_request->fki_employee_id = NULL;
    }
    if (user_request->fki_company_id_default) {
        free(user_request->fki_company_id_default);
        user_request->fki_company_id_default = NULL;
    }
    if (user_request->fki_department_id_default) {
        free(user_request->fki_department_id_default);
        user_request->fki_department_id_default = NULL;
    }
    if (user_request->fki_timezone_id) {
        free(user_request->fki_timezone_id);
        user_request->fki_timezone_id = NULL;
    }
    if (user_request->fki_language_id) {
        free(user_request->fki_language_id);
        user_request->fki_language_id = NULL;
    }
    ...
    if (user_request->fki_billingentityinternal_id) {
        free(user_request->fki_billingentityinternal_id);
        user_request->fki_billingentityinternal_id = NULL;
    }
    ...
    if (user_request->fki_secretquestion_id) {
        free(user_request->fki_secretquestion_id);
        user_request->fki_secretquestion_id = NULL;
    }
    ...
    if (user_request->fki_module_id_form) {
        free(user_request->fki_module_id_form);
        user_request->fki_module_id_form = NULL;
    }
    ...
    if (user_request->b_user_isactive) {
        free(user_request->b_user_isactive);
        user_request->b_user_isactive = NULL;
    }
    if (user_request->b_user_validatebyadministration) {
        free(user_request->b_user_validatebyadministration);
        user_request->b_user_validatebyadministration = NULL;
    }
    if (user_request->b_user_validatebydirector) {
        free(user_request->b_user_validatebydirector);
        user_request->b_user_validatebydirector = NULL;
    }
    if (user_request->b_user_attachmentautoverified) {
        free(user_request->b_user_attachmentautoverified);
        user_request->b_user_attachmentautoverified = NULL;
    }
    if (user_request->b_user_changepassword) {
        free(user_request->b_user_changepassword);
        user_request->b_user_changepassword = NULL;
    }
    if (user_request->b_user_ezsigntemplaterolegrouping) {
        free(user_request->b_user_ezsigntemplaterolegrouping);
        user_request->b_user_ezsigntemplaterolegrouping = NULL;
    }
    free(user_request);
}
...
cJSON *user_request_convertToJSON(user_request_t *user_request) {
    ...
    // user_request->b_user_ezsigntemplaterolegrouping
    if(user_request->b_user_ezsigntemplaterolegrouping) {
    if(cJSON_AddBoolToObject(item, "bUserEzsigntemplaterolegrouping", *user_request->b_user_ezsigntemplaterolegrouping) == NULL) {
    goto fail; //Bool
    }
    }
...
user_request_t *user_request_parseFromJSON(cJSON *user_requestJSON){
    ...
    // define the local variable for user_request->pki_user_id
    int *pki_user_id_local_var = NULL;

    // define the local variable for user_request->fki_agent_id
    int *fki_agent_id_local_var = NULL;

    // define the local variable for user_request->fki_broker_id
    int *fki_broker_id_local_var = NULL;

    // define the local variable for user_request->fki_assistant_id
    int *fki_assistant_id_local_var = NULL;

    // define the local variable for user_request->fki_employee_id
    int *fki_employee_id_local_var = NULL;

    // define the local variable for user_request->fki_company_id_default
    int *fki_company_id_default_local_var = NULL;

    // define the local variable for user_request->fki_department_id_default
    int *fki_department_id_default_local_var = NULL;

    // define the local variable for user_request->fki_timezone_id
    int *fki_timezone_id_local_var = NULL;

    // define the local variable for user_request->fki_language_id
    ...
    // define the local variable for user_request->fki_billingentityinternal_id
    ...
    // define the local variable for user_request->fki_secretquestion_id
    int *fki_secretquestion_id_local_var = NULL;

    char *s_user_secretresponse_local_str = NULL;

    // define the local variable for user_request->fki_module_id_form
    ...
    // define the local variable for user_request->b_user_isactive
    int *b_user_isactive_local_var = NULL;

    // define the local variable for user_request->b_user_validatebyadministration
    int *b_user_validatebyadministration_local_var = NULL;

    // define the local variable for user_request->b_user_validatebydirector
    int *b_user_validatebydirector_local_var = NULL;

    // define the local variable for user_request->b_user_attachmentautoverified
    int *b_user_attachmentautoverified_local_var = NULL;

    // define the local variable for user_request->b_user_changepassword
    int *b_user_changepassword_local_var = NULL;

    // define the local variable for user_request->b_user_ezsigntemplaterolegrouping
    ...
    *b_user_changepassword_local_var = b_user_changepassword->valueint;
    ...
    // user_request->b_user_ezsigntemplaterolegrouping
    cJSON *b_user_ezsigntemplaterolegrouping = cJSON_GetObjectItemCaseSensitive(user_requestJSON, "bUserEzsigntemplaterolegrouping");
