...
static subnet_request_compound_t *subnet_request_compound_create_internal(
    ...
    memset(subnet_request_compound_local_var, 0, sizeof(subnet_request_compound_t));
...
__attribute__((deprecated)) subnet_request_compound_t *subnet_request_compound_create(
    ...
    subnet_request_compound_t *result = subnet_request_compound_create_internal (
        pki_subnet_id_copy,
        fki_user_id_copy,
        fki_apikey_id_copy,
...
void subnet_request_compound_free(subnet_request_compound_t *subnet_request_compound) {
    ...
    if (subnet_request_compound->pki_subnet_id) {
        free(subnet_request_compound->pki_subnet_id);
        subnet_request_compound->pki_subnet_id = NULL;
    }
    if (subnet_request_compound->fki_user_id) {
        free(subnet_request_compound->fki_user_id);
        subnet_request_compound->fki_user_id = NULL;
    }
    if (subnet_request_compound->fki_apikey_id) {
        free(subnet_request_compound->fki_apikey_id);
        subnet_request_compound->fki_apikey_id = NULL;
    }
    ...
    if (subnet_request_compound->i_subnet_network) {
        free(subnet_request_compound->i_subnet_network);
        subnet_request_compound->i_subnet_network = NULL;
    }
    if (subnet_request_compound->i_subnet_mask) {
        free(subnet_request_compound->i_subnet_mask);
        subnet_request_compound->i_subnet_mask = NULL;
    }
...
subnet_request_compound_t *subnet_request_compound_parseFromJSON(cJSON *subnet_request_compoundJSON){
    ...
    // define the local variable for subnet_request_compound->pki_subnet_id
    int *pki_subnet_id_local_var = NULL;

    // define the local variable for subnet_request_compound->fki_user_id
    int *fki_user_id_local_var = NULL;

    // define the local variable for subnet_request_compound->fki_apikey_id
    ...
    // define the local variable for subnet_request_compound->i_subnet_network
    long *i_subnet_network_local_var = NULL;

    // define the local variable for subnet_request_compound->i_subnet_mask
