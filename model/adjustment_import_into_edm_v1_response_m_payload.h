/*
 * adjustment_import_into_edm_v1_response_m_payload.h
 *
 * Payload for POST /1/object/adjustment/{pkiAdjustmentID}/importIntoEDM
 */

#ifndef _adjustment_import_into_edm_v1_response_m_payload_H_
#define _adjustment_import_into_edm_v1_response_m_payload_H_

#include <string.h>
#include "../external/cJSON.h"
#include "../include/list.h"
#include "../include/keyValuePair.h"
#include "../include/binary.h"

typedef struct adjustment_import_into_edm_v1_response_m_payload_t adjustment_import_into_edm_v1_response_m_payload_t;

#include "custom_attachment_import_into_edm_response.h"



typedef struct adjustment_import_into_edm_v1_response_m_payload_t {
    list_t *a_obj_attachment; //nonprimitive container

    int _library_owned; // Is the library responsible for freeing this object?
} adjustment_import_into_edm_v1_response_m_payload_t;

__attribute__((deprecated)) adjustment_import_into_edm_v1_response_m_payload_t *adjustment_import_into_edm_v1_response_m_payload_create(
    list_t *a_obj_attachment
);

void adjustment_import_into_edm_v1_response_m_payload_free(adjustment_import_into_edm_v1_response_m_payload_t *adjustment_import_into_edm_v1_response_m_payload);

adjustment_import_into_edm_v1_response_m_payload_t *adjustment_import_into_edm_v1_response_m_payload_parseFromJSON(cJSON *adjustment_import_into_edm_v1_response_m_payloadJSON);

cJSON *adjustment_import_into_edm_v1_response_m_payload_convertToJSON(adjustment_import_into_edm_v1_response_m_payload_t *adjustment_import_into_edm_v1_response_m_payload);

#endif /* _adjustment_import_into_edm_v1_response_m_payload_H_ */

