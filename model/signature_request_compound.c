...
static signature_request_compound_t *signature_request_compound_create_internal(
    ...
    memset(signature_request_compound_local_var, 0, sizeof(signature_request_compound_t));
...
__attribute__((deprecated)) signature_request_compound_t *signature_request_compound_create(
    ...
    signature_request_compound_t *result = signature_request_compound_create_internal (
...
void signature_request_compound_free(signature_request_compound_t *signature_request_compound) {
    ...
    if (signature_request_compound->pki_signature_id) {
        free(signature_request_compound->pki_signature_id);
        signature_request_compound->pki_signature_id = NULL;
    }
    if (signature_request_compound->fki_font_id) {
        free(signature_request_compound->fki_font_id);
        signature_request_compound->fki_font_id = NULL;
    }
...
signature_request_compound_t *signature_request_compound_parseFromJSON(cJSON *signature_request_compoundJSON){
    ...
    // define the local variable for signature_request_compound->pki_signature_id
    int *pki_signature_id_local_var = NULL;

    // define the local variable for signature_request_compound->fki_font_id
