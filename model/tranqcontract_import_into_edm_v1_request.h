/*
 * tranqcontract_import_into_edm_v1_request.h
 *
 * Request for POST /1/object/tranqcontract/{pkiTranqcontractID}/importIntoEDM
 */

#ifndef _tranqcontract_import_into_edm_v1_request_H_
#define _tranqcontract_import_into_edm_v1_request_H_

#include <string.h>
#include "../external/cJSON.h"
#include "../include/list.h"
#include "../include/keyValuePair.h"
#include "../include/binary.h"

typedef struct tranqcontract_import_into_edm_v1_request_t tranqcontract_import_into_edm_v1_request_t;

#include "custom_attachment_import_into_edm_request.h"



typedef struct tranqcontract_import_into_edm_v1_request_t {
    list_t *a_obj_attachment; //nonprimitive container

    int _library_owned; // Is the library responsible for freeing this object?
} tranqcontract_import_into_edm_v1_request_t;

__attribute__((deprecated)) tranqcontract_import_into_edm_v1_request_t *tranqcontract_import_into_edm_v1_request_create(
    list_t *a_obj_attachment
);

void tranqcontract_import_into_edm_v1_request_free(tranqcontract_import_into_edm_v1_request_t *tranqcontract_import_into_edm_v1_request);

tranqcontract_import_into_edm_v1_request_t *tranqcontract_import_into_edm_v1_request_parseFromJSON(cJSON *tranqcontract_import_into_edm_v1_requestJSON);

cJSON *tranqcontract_import_into_edm_v1_request_convertToJSON(tranqcontract_import_into_edm_v1_request_t *tranqcontract_import_into_edm_v1_request);

#endif /* _tranqcontract_import_into_edm_v1_request_H_ */

