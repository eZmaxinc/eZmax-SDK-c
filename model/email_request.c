...
static email_request_t *email_request_create_internal(
    ...
    memset(email_request_local_var, 0, sizeof(email_request_t));
...
__attribute__((deprecated)) email_request_t *email_request_create(
    ...
    email_request_t *result = email_request_create_internal (
        pki_email_id_copy,
...
void email_request_free(email_request_t *email_request) {
    ...
    if (email_request->pki_email_id) {
        free(email_request->pki_email_id);
        email_request->pki_email_id = NULL;
    }
    if (email_request->fki_emailtype_id) {
        free(email_request->fki_emailtype_id);
        email_request->fki_emailtype_id = NULL;
    }
...
email_request_t *email_request_parseFromJSON(cJSON *email_requestJSON){
    ...
    // define the local variable for email_request->pki_email_id
    int *pki_email_id_local_var = NULL;

    // define the local variable for email_request->fki_emailtype_id
