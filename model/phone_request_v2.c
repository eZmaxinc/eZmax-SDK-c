...
static phone_request_v2_t *phone_request_v2_create_internal(
    int *pki_phone_id,
    ...
    memset(phone_request_v2_local_var, 0, sizeof(phone_request_v2_t));
    phone_request_v2_local_var->_library_owned = 1;
...
__attribute__((deprecated)) phone_request_v2_t *phone_request_v2_create(
    ...
    phone_request_v2_t *result = phone_request_v2_create_internal (
        pki_phone_id_copy,
...
void phone_request_v2_free(phone_request_v2_t *phone_request_v2) {
    ...
    if(phone_request_v2->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "phone_request_v2_free");
        return ;
    }
    listEntry_t *listEntry;
    if (phone_request_v2->pki_phone_id) {
        free(phone_request_v2->pki_phone_id);
        phone_request_v2->pki_phone_id = NULL;
    }
    if (phone_request_v2->fki_phonetype_id) {
        free(phone_request_v2->fki_phonetype_id);
        phone_request_v2->fki_phonetype_id = NULL;
    }
...
phone_request_v2_t *phone_request_v2_parseFromJSON(cJSON *phone_request_v2JSON){
    ...
    // define the local variable for phone_request_v2->pki_phone_id
    int *pki_phone_id_local_var = NULL;

    // define the local variable for phone_request_v2->fki_phonetype_id
    ...
    *fki_phonetype_id_local_var = fki_phonetype_id->valuedouble;
    ...
    phone_request_v2_local_var = phone_request_v2_create_internal (
