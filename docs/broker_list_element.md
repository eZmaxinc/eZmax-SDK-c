# broker_list_element_t

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**pki_broker_id** | **int** | The unique ID of the Broker. | 
**fki_department_id** | **int** | The unique ID of the Department | 
**s_department_name_x** | **char \*** | The Name of the Department in the language of the requester | [optional] 
**fki_brokertype_id** | **int** | The unique ID of the Brokertype | 
**s_brokertype_name_x** | **char \*** | The name of the Brokertype in the language of the requester | 
**s_broker_code** | **char \*** | The code of the Broker | 
**s_realestateboardnumber_number** | **char \*** | The number of the Realestateboardnumber | [optional] 
**i_agent_bannernumber** | **int** | The bannernumber of the Agent | [optional] 
**s_language_name_x** | **char \*** | The Name of the Language in the language of the requester | [optional] 
**i_broker_photocopiercode** | **int** | The photocopiercode of the Broker | 
**i_broker_longdistancecode** | **int** | The longdistancecode of the Broker | 
**s_broker_name** | **char \*** | The name of the Broker | 
**s_broker_realestateassociationlicense** | **char \*** | The realestateassociationlicense of the Broker | 
**dt_broker_hiredate** | **char \*** | The hiredate of the Broker | 
**dt_broker_leavedate** | **char \*** | The leavedate of the Broker | [optional] 
**b_broker_tranquillit** | **int** | Whether if it&#39;s an tranquillit | [optional] 
**b_broker_residentiallicense** | **int** | Whether if it&#39;s an residentiallicense | 
**b_broker_commerciallicense** | **int** | Whether if it&#39;s an commerciallicense | 
**b_broker_mortgagelicense** | **int** | Whether if it&#39;s an mortgagelicense | 
**b_broker_paidbyofficetranquillit** | **int** | Whether if it&#39;s an paidbyofficetranquillit | 
**dt_broker_fintraccertification** | **char \*** | The fintraccertification of the Broker | [optional] 
**b_broker_isactive** | **int** | Whether the Broker is active or not | 
**s_contact_firstname** | **char \*** | The First name of the contact | [optional] 
**s_contact_lastname** | **char \*** | The Last name of the contact | [optional] 
**dt_contact_birthdate** | **char \*** | The Birth Date of the contact | [optional] 
**s_email_address** | **char \*** | The email address. | [optional] 
**s_phone_e164** | **char \*** | A phone number in E.164 Format | [optional] 
**s_address_civic** | **char \*** | The Civic number. | [optional] 
**s_address_street** | **char \*** | The Street Name | [optional] 
**s_address_suite** | **char \*** | The Suite or appartment number | [optional] 
**s_address_city** | **char \*** | The City name | [optional] 
**s_address_zip** | **char \*** | The Postal/Zip Code  The value must be entered without spaces | [optional] 
**s_province_name_x** | **char \*** | The name of the Province in the language of the requester | [optional] 
**s_country_name_x** | **char \*** | The name of the Country in the language of the requester | [optional] 

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


