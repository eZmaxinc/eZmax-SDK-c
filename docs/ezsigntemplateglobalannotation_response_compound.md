# ezsigntemplateglobalannotation_response_compound_t

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**pki_ezsigntemplateglobalannotation_id** | **int** | The unique ID of the Ezsigntemplateglobalannotation | 
**fki_textstylestatic_id** | **int** | The unique ID of the Textstylestatic | [optional] 
**obj_textstylestatic** | [**textstylestatic_request_compound_t**](textstylestatic_request_compound.md) \* |  | [optional] 
**e_ezsigntemplateglobalannotation_horizontalalignment** | **field_e_ezsigntemplateglobalannotation_horizontalalignment_t \*** |  | 
**e_ezsigntemplateglobalannotation_verticalalignment** | **field_e_ezsigntemplateglobalannotation_verticalalignment_t \*** |  | 
**e_ezsigntemplateglobalannotation_type** | **field_e_ezsigntemplateglobalannotation_type_t \*** |  | 
**i_ezsigntemplateglobalannotation_x** | **int** | The x of the Ezsigntemplateglobalannotation | 
**i_ezsigntemplateglobalannotation_y** | **int** | The y of the Ezsigntemplateglobalannotation | 
**i_ezsigntemplateglobalannotation_width** | **int** | The width of the Ezsigntemplateglobalannotation | 
**i_ezsigntemplateglobalannotation_height** | **int** | The height of the Ezsigntemplateglobalannotation | 
**i_ezsigntemplateglobaldocumentpage_pagenumber** | **int** | The page number in the Ezsigntemplateglobaldocument | 
**s_ezsigntemplateglobalannotation_description** | **char \*** | The description of the Ezsigntemplateglobalannotation | 
**s_ezsigntemplateglobalannotation_defaulttext** | **char \*** | The defaulttext of the Ezsigntemplateglobalannotation | 
**s_ezsigntemplateglobalannotation_dropdownvalues** | **char \*** | The dropdownvalues of the Ezsigntemplateglobalannotation | 

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


